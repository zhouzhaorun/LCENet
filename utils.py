...
def validation(net, val_data_loader, device, epoch, save_valid_dir):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        mkdir_fun(save_valid_dir + 'image_out')
        mkdir_fun(save_valid_dir + 'gradient_in')
        mkdir_fun(save_valid_dir + 'gradient_out')
        mkdir_fun(save_valid_dir + 'light_in')
        mkdir_fun(save_valid_dir + 'light_out')
...
def t_model(net, val_data_loader, device, save_dir):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        with torch.no_grad():
            ...
            if batch_id != 0:
                timelist += end - start
            print("image: %d\ttime: %.3f" % (batch_id, end-start))
        img_ = transform_invert(delow)

        mkdir_fun(save_dir + 'image_out')
        mkdir_fun(save_dir + 'gradient_in')
        mkdir_fun(save_dir + 'gradient_out')
        mkdir_fun(save_dir + 'light_in')
        mkdir_fun(save_dir + 'light_out')

        img_.save(save_dir + 'image_out' + '/' + img_name[0])
        tin_ = transform_invert(torch.mean(tin, dim=1, keepdim=True))
        tin_.save(save_dir + 'gradient_in' + '/' + img_name[0])
        tout_ = transform_invert(torch.mean(tout, dim=1, keepdim=True))
    ...
    print("time mean : %.3f" % (timelist / len(val_data_loader)))
